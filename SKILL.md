...
1. Oynatma Performansı (Hibrit Decoder Fix):
   •   Build 18'de ortaya çıkan "görüntü takılması" ve "hızlı oynatma" sorunları giderildi.
   •   Build 14'teki kararlı "Hibrit Decoder" (Önce Donanım, Başarısız Olursa Yazılım) mantığına geri dönüldü (EXTENSION_RENDERER_MODE_ON).
   •   Ses ve görüntü senkronizasyonu için LoadControl ve TrackSelector ayarları optimize edildi.
2. Düşük Donanım Uyumluluğu:
   •   Zayıf cihazlarda yazılımsal dekoderler hala bir seçenek (fallback) olarak aktif kalmaya devam ediyor, ancak donanım dekoderi varsa artık öncelikli olarak o kullanılıyor.
3. Stabilite:
   •   Kanal geçişlerindeki (Zapping) bellek yönetimi ve kaynak temizleme süreçleri daha kararlı hale getirildi.